...
stmt :: P Stmt
stmt = pMetaStmt <|> ifStmt <|> whileStmt <|> forStmt <|> labeledStmt <|> stmtNoTrail
  where
    ifStmt = do
        tok KW_If
        e   <- parens exp
...
pMetaVar = tok MetaVarTok >> (javaToken $ \t -> case t of IdentTok s -> Just $ EMetaVar s
                                                          _ -> Nothing)
pMetaStmt = tok MetaStmtTok >> (javaToken $ \t -> case t of IdentTok s -> Just $ EMetaStmt s
                                                            _ -> Nothing)

-- small   = lower <|> char '_'
-- large   = upper
-- idchar  = small <|> large <|> digit <|> char '\''
-- mident = do { c <- small; cs <- many idchar; return (c:cs) }

ident :: P Ident
ident = pMetaVar <|> jid
    where jid = javaToken $ \t -> case t of IdentTok s -> Just $ Ident s
                                            _ -> Nothing
